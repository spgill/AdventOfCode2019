...
def main(input_file):
    ...
    hullRobotProgram = input_file.read().strip()

    # Create a new IntCode machine to run the program
    machine = IntCodeMachine(hullRobotProgram)

    # Variables to track robot actions
    paintedTiles = set()
    tally = 0
    tileColors = collections.defaultdict(lambda: TileColor.BLACK)
    positionX = 0
    positionY = 0
    direction = 0

    # Make the starting panel white
    tileColors[(0, 0)] = TileColor.WHITE

    # Start the execution loop
    while machine.state is not IntCodeMachineState.HALTED:
        # Input the current tile color
        machine.inputValue = tileColors[(positionX, positionY)].value

        # Execute until the next interrupt
        machine.execute()

        # Handle any output values
        for i, value in enumerate(machine.outputValues):
            # First output values is the paint command
            if i == 0:
                paintedTiles.add((positionX, positionY))
                tally += 1
                tileColors[(positionX, positionY)] = TileColor(value)

            # Second output value is rotation
            if i == 1:
                # Change angle
                if value == 0:
        ...
        # Wipe the output
        machine.outputValues = []

    # Determine the boundaries of the drawing
    xMin = min(paintedTiles, key=lambda coord: coord[0])[0]
    xMax = max(paintedTiles, key=lambda coord: coord[0])[0]
    yMin = min(paintedTiles, key=lambda coord: coord[1])[1]
    yMax = max(paintedTiles, key=lambda coord: coord[1])[1]

    # Map the tile colors to characters
    tileCharacters = {TileColor.BLACK: " ", TileColor.WHITE: "█"}

    # Iterate through each tile in the image and print to console.
    print("RESULT:")
    for y in range(yMax, yMin - 1, -1):  # The y axis is inverted.
        for x in range(xMin, xMax + 1):
            value = tileColors[(x, y)]
            sys.stdout.write(tileCharacters[value])
        sys.stdout.write("\n")
