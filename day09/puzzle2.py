...
def main(input_file):
    ...
    # Load the boost program from the input file
    boostProgram = input_file.read().strip()

    # Load the program into a new intcode machine
    machine = IntCodeMachine(boostProgram)

    # Run the program in sensor boost mode
    machine.inputValue = 2
    machine.execute()

    # Result is the machine's output value
    print("RESULT:", machine.outputValues[0])
